...
class DefaultContainer(DataContainer[t.Any, t.List[t.Any]]):
    ...
    def batch_to_batches(
        ...
        # Optimize by precomputing slice pairs and using list comprehension with zip and itertools.pairwise for better cache locality and less attribute lookups.
        # However, pairwise is only in Python 3.10+, so using zip(indices, indices[1:]).
        idx = indices
        return [batch[start:end] for start, end in zip(idx, idx[1:])]
